...
기능 구현과 검수가 끝나면 변경 내용과 검수 결과를 요약하고,
사용자 승인 뒤 해당 기능만 하나의 커밋으로 기록한다.
구현과 전체 검수가 끝난 뒤, 사용자 승인과 배포 요청이 있을 때만
`dev`를 `main`으로 병합하고 push한다.
일상 작업 중에는 `main`에 병합하거나 push하지 않는다.
